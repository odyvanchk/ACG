...
//                double currZ = evaluateZ(j, i, p1i[0], p1i[1], p1i[2], p2i[0], p2i[1], p2i[2], p3i[0], p3i[1], p3i[2]);
    ...
    private double evaluateZ(int x, int y, int x1, int y1, double z1, int x2, int y2, double z2, int x3, int y3, double z3) {
        double z = 1000.0;
        if (Math.abs(((x2 - x1) * (y3 - y1) - (x3 - x1) * (y2 - y1))) >= 1) {
//        if (Math.abs((x3 - x1) / (x2 - x1) - (y3 - y1) / (y2 - y1)) >= 0.001) {
            z = -((((double) x - x1) * ((y2 - y1) * (z3 - z1) - (y3 - y1) * (z2 - z1)))
